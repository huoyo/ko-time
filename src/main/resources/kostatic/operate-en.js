    ...
    $.get(contextPath+'/koTime/getApis?question='+searchText+'&token='+globalToken+"&project="+globalProject, function (data) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (methodType=='Controller' && routeName!=null && routeName!='') {
                ...
                html += "<li onclick=\"showMethods('"+id+"')\" style='color: #333;font-weight: 400;font-size: 14px;' id=\""+apiId+"-list\">"+ className+"#<span style='font-size: 16px;font-weight: 500;'>"+methodName+"</span>&nbsp &nbsp<span style='font-size: 10px;' class=\"uk-label uk-label-"+color+"\">avg "+avgRunTime+" ms</span></li>";
            }
        };
